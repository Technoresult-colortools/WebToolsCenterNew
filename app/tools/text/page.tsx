...
import { Search, Type, Wand2, Sparkles, ArrowRight } from "lucide-react"
...
export default function TextToolsPage() {
  ...
  // Filter only text category tools
  const textTools = allTools.filter((tool) => tool.category === "Text")

  const filteredTools = textTools.filter(
    (tool) =>
        ...
        >
          <div className="relative mb-6">
            <span className="inline-block animate-bounce mb-4">
              <FontAwesomeIcon icon={faFont} className="w-16 h-16 text-primary opacity-75" />
            </span>
            <h1 className="text-5xl md:text-7xl font-bold text-foreground tracking-tight">
              Text <span className="text-primary">Tools</span>
            </h1>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredTools.length > 0 ? (
            ...
            <div className="col-span-full text-center py-12">
              <FontAwesomeIcon icon={faFont} className="w-16 h-16 text-default-300 mx-auto mb-4" />
              <p className="text-xl text-default-600">
                No text tools found matching your criteria. Try adjusting your search.
              </p>
            </div>
          )}
        </div>
            ...
            {[
              {
                ...
                description: "Advanced algorithms for intelligent text manipulation",
              },
              {
                icon: <Type className="w-10 h-10 text-success" />,
                title: "Format Flexibility",
                description: "Support for multiple text formats and encodings",
              },
              {
                icon: <Wand2 className="w-10 h-10 text-primary" />,
                title: "Real-time Preview",
                description: "Instant results as you type with live previews",
              },
            ].map((feature, index) => (
              <Card
                key={index}
