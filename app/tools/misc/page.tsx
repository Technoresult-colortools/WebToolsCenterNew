...
import { Search, PenToolIcon as Tool, Wand2, Sparkles, ArrowRight } from "lucide-react"
...
export default function MiscToolsPage() {
  ...
  // Filter only Misc category tools
  const miscTools = allTools.filter((tool) => tool.category === "Misc")

  const filteredTools = miscTools.filter(
    (tool) =>
        ...
        >
          <div className="relative mb-6">
            <span className="inline-block animate-bounce mb-4">
              <FontAwesomeIcon icon={faWrench} className="w-16 h-16 text-primary opacity-75" />
            </span>
            <h1 className="text-5xl md:text-7xl font-bold text-foreground tracking-tight">
              Misc <span className="text-primary">Tools</span>
            </h1>
          </div>
          <p className="text-xl text-default-600 mb-12 max-w-2xl mx-auto leading-relaxed">
            Discover a variety of useful tools and utilities to enhance your productivity and solve diverse challenges.
          </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredTools.length > 0 ? (
              ...
              >
                <CardBody className="p-6">
                  <div className="flex items-start gap-4">
                    <div
                      className={`w-12 h-12 bg-gradient-to-br ${categories.find((cat) => cat.name === "Misc")?.gradient} rounded-xl flex items-center justify-center transform transition-all duration-500 ease-out
            ...
            <div className="col-span-full text-center py-12">
              <FontAwesomeIcon icon={faWrench} className="w-16 h-16 text-default-300 mx-auto mb-4" />
              <p className="text-xl text-default-600">
                No misc tools found matching your criteria. Try adjusting your search.
              </p>
            </div>
          )}
        </div>
      ...
      <section className="py-24 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 relative">
          <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-16 text-center flex items-center justify-center">
            ...
            Why Use Our Misc Tools?
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
            {[
              {
                icon: <Tool className="w-10 h-10 text-warning" />,
                title: "Versatility",
                description: "A diverse set of tools to tackle various tasks and challenges",
              },
              {
                icon: <Sparkles className="w-10 h-10 text-success" />,
                title: "Efficiency",
                description: "Save time and effort with our carefully crafted utilities",
              },
              {
                icon: <Wand2 className="w-10 h-10 text-primary" />,
                title: "Problem-Solving",
                description: "Find creative solutions to unique problems with our misc tools",
              },
            ].map((feature, index) => (
              <Card
                key={index}
