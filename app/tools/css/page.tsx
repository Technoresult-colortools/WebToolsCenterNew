...
import { Search, Palette, Wand2, Sparkles, ArrowRight } from "lucide-react"
...
export default function CSSToolsPage() {
  ...
  // Filter only CSS category tools
  const cssTools = allTools.filter((tool) => tool.category === "CSS")

  const filteredTools = cssTools.filter(
    (tool) =>
        ...
        >
          <div className="relative mb-6">
            <span className="inline-block animate-bounce mb-4">
              <FontAwesomeIcon icon={faPalette} className="w-16 h-16 text-primary opacity-75" />
            </span>
            <h1 className="text-5xl md:text-7xl font-bold text-foreground tracking-tight">
              CSS <span className="text-primary">Tools</span>
            </h1>
          </div>
          <p className="text-xl text-default-600 mb-12 max-w-2xl mx-auto leading-relaxed">
            Create, optimize, and enhance your stylesheets with our powerful collection of CSS tools.
          </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredTools.length > 0 ? (
              ...
              >
                <CardBody className="p-6">
                  <div className="flex items-start gap-4">
                    <div
                      className={`w-12 h-12 bg-gradient-to-br ${categories.find((cat) => cat.name === "CSS")?.gradient} rounded-xl flex items-center justify-center transform transition-all duration-500 ease-out
            ...
            <div className="col-span-full text-center py-12">
              <FontAwesomeIcon icon={faPalette} className="w-16 h-16 text-default-300 mx-auto mb-4" />
              <p className="text-xl text-default-600">
                No CSS tools found matching your criteria. Try adjusting your search.
              </p>
            </div>
          )}
        </div>
      ...
      <section className="py-24 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 relative">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
            {[
              {
                icon: <Sparkles className="w-10 h-10 text-warning" />,
                title: "Effortless Styling",
                description: "Create complex CSS styles with just a few clicks",
              },
              {
                icon: <Palette className="w-10 h-10 text-success" />,
                title: "Visual Customization",
                description: "Intuitive interfaces for crafting beautiful designs",
              },
              {
                icon: <Wand2 className="w-10 h-10 text-primary" />,
                title: "Code Generation",
                description: "Instantly generate clean, optimized CSS code",
              },
            ].map((feature, index) => (
              <Card
                key={index}
