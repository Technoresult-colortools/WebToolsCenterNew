...
import { Search, Image, Wand2, Sparkles, ArrowRight } from "lucide-react"
...
export default function ImageToolsPage() {
  ...
  // Filter only image category tools
        ...
        >
          <div className="relative mb-6">
            <span className="inline-block animate-bounce mb-4">
              <FontAwesomeIcon icon={faImage} className="w-16 h-16 text-primary opacity-75" />
            </span>
            <h1 className="text-5xl md:text-7xl font-bold text-foreground tracking-tight">
              Image <span className="text-primary">Tools</span>
            </h1>
          </div>
          <p className="text-xl text-default-600 mb-12 max-w-2xl mx-auto leading-relaxed">
            Edit, enhance, and transform your images with our powerful collection of image manipulation tools.
          </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredTools.length > 0 ? (
              ...
              >
                <CardBody className="p-6">
                  <div className="flex items-start gap-4">
                    <div
                      className={`w-12 h-12 bg-gradient-to-br ${categories.find((cat) => cat.name === "Image")?.gradient} rounded-xl flex items-center justify-center transform transition-all duration-500 ease-out
            ...
            <div className="col-span-full text-center py-12">
              <FontAwesomeIcon icon={faImage} className="w-16 h-16 text-default-300 mx-auto mb-4" />
              <p className="text-xl text-default-600">
                No image tools found matching your criteria. Try adjusting your search.
              </p>
            </div>
          )}
        </div>
            ...
            {[
              {
                ...
                description: "Powerful algorithms for sophisticated image manipulation",
              },
              {
                icon: <Image className="w-10 h-10 text-success" />,
                title: "Format Support",
                description: "Wide range of supported image formats for versatile editing",
              },
              {
                icon: <Wand2 className="w-10 h-10 text-primary" />,
                title: "Real-time Preview",
                description: "Instant visual feedback as you edit your images",
              },
            ].map((feature, index) => (
              <Card
                key={index}
