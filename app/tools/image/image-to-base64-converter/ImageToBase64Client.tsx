            ...
            <Tab key="upload" title="Upload">
              <Card className="bg-default-50 dark:bg-default-100">
                <CardBody className="p-6" >
                  ...
                  {error && (
                    <div className="mt-4 p-4 bg-danger text-danger-foreground rounded-lg flex items-center">
                ...
                <Card className="bg-default-50 dark:bg-default-200">
                  <CardHeader className="flex justify-between items-center">
                    ...
                    <div className="flex space-x-2">
                      <Dropdown>
                        <DropdownTrigger>
                          <Button variant="flat">Sort By</Button>
                        </DropdownTrigger>
                        <DropdownMenu
                      ...
                      <Button color="success" onClick={downloadAllBase64}>
                        ...
                        <Card className="bg-default-50 dark:bg-default-100" key={index}>
                          <CardBody>
                            <div className="flex justify-between items-center mb-2">
                              ...
                              <Button isIconOnly color="danger" variant="light" onClick={() => removeImage(index)}>
                                <X size={20} />
                              </Button>
                            </div>
                            <div className="flex items-center mb-2">
                              <ImageIcon className="w-5 h-5 mr-2" />
                              <span className="text-sm">{(img.size / 1024).toFixed(2)} KB</span>
                              <FileText className="w-5 h-5 ml-4 mr-2" />
                              <span className="text-sm">{img.type}</span>
                            </div>
                            <div className="flex space-x-2 mb-4">
                              <Button color="primary" onClick={() => copyToClipboard(img.base64)}>
                                <Copy size={14} className="mr-2" />
                                Copy Base64
                              </Button>
                              <Button color="secondary" onClick={() => downloadBase64(img.base64, img.name)}>
                                <Download size={14} className="mr-2" />
                                Download Base64
                              </Button>
                            </div>
                            <div className="relative h-40 bg-default-100 rounded-lg overflow-hidden">
                              <NextImage src={img.base64} alt={img.name} layout="fill" objectFit="contain" />
                            </div>
                            <div className="mt-2">
                              <h4 className="text-sm font-semibold mb-1">Base64 Preview:</h4>
                              <p className="text-xs break-all">{img.base64.substring(0, 100)}...</p>
                            </div>
                          </CardBody>
                        </Card>
                      ))}
