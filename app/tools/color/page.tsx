...
import { Search, Droplet, Wand2, Sparkles, ArrowRight } from "lucide-react"
...
export default function ColorToolsPage() {
  ...
  // Filter only Color category tools
  const colorTools = allTools.filter((tool) => tool.category === "Color")

  const filteredTools = colorTools.filter(
    (tool) =>
        ...
        >
          <div className="relative mb-6">
            <span className="inline-block animate-bounce mb-4">
              <FontAwesomeIcon icon={faDroplet} className="w-16 h-16 text-primary opacity-75" />
            </span>
            <h1 className="text-5xl md:text-7xl font-bold text-foreground tracking-tight">
              Color <span className="text-primary">Tools</span>
            </h1>
          </div>
          <p className="text-xl text-default-600 mb-12 max-w-2xl mx-auto leading-relaxed">
            Explore, create, and manipulate colors with our comprehensive suite of color tools and utilities.
          </p>
          ...
          <div className="max-w-4xl mx-auto mb-12">
            <div className="flex gap-4 flex-col sm:flex-row">
              <Input
                placeholder="Search color tools..."
                value={searchQuery}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredTools.length > 0 ? (
              ...
              >
                <CardBody className="p-6">
                  <div className="flex items-start gap-4">
                    <div
                      className={`w-12 h-12 bg-gradient-to-br ${categories.find((cat) => cat.name === "Color")?.gradient} rounded-xl flex items-center justify-center transform transition-all duration-500 ease-out
            ...
            <div className="col-span-full text-center py-12">
              <FontAwesomeIcon icon={faDroplet} className="w-16 h-16 text-default-300 mx-auto mb-4" />
              <p className="text-xl text-default-600">
                No color tools found matching your criteria. Try adjusting your search.
              </p>
            </div>
          )}
        </div>
      ...
      <section className="py-24 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 relative">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
            {[
              {
                icon: <Droplet className="w-10 h-10 text-warning" />,
                title: "Color Exploration",
                description: "Discover and experiment with a wide range of color combinations",
              },
              {
                icon: <Sparkles className="w-10 h-10 text-success" />,
                title: "Accessibility Focus",
                description: "Ensure your color choices meet accessibility standards",
              },
              {
                icon: <Wand2 className="w-10 h-10 text-primary" />,
                title: "Design Integration",
                description: "Seamlessly integrate colors into your design workflow",
              },
            ].map((feature, index) => (
              <Card
                key={index}
